...
class EUnet(nn.Module):
    ...
    def __init__(self, in_dims, out_dims, k=1,s=1,l=1,depth=3,base=8,init_xavier=False, padding=0,non_linearity = nn.ELU,use_dropout=False,use_bn=False):
        """
        ...
        for i in range(self.depth):
            ...
            dn = DownModule(base*(2**i)*k, base*(2**(i+1))*k,s,non_linearity=non_linearity,padding=padding,use_dropout=use_dropout,use_bn=use_bn)
            up = UpModule(base*(2**(i+1))*k, base*(2**i)*k,l,non_linearity=non_linearity,padding=padding)
